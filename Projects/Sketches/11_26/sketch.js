...
var maxLevel = 10; // The amount of nested branches it will subdivide to. More is slower!
var branchForce = 2; // The branch's resistance against the mouse. A lower value will make it feel sluggish, while a bigger value will make it spring-like.
var rootBranches = [];
var debug = false;
var count = 0;
...
function treeIterator(branch, worldX, worldY, worldA) {
  ...
  push();
    stroke(map(branch.level, 0, maxLevel, 220, 20));
    strokeWeight(map(branch.level, 0, maxLevel, 1.5, 0.2));
