var t;
var theta;
var maxFrameCount = 200; // frameCount, change for faster or slower animation

function setup() {
  createCanvas(600, 600);
  noStroke();
  background(0)
}

function draw() {
  background(0, 15);
    fill(255);
  text("5.11.20", 30, height - 30)
  translate(width/2, height/2); // translate 0,0 to center

  t = frameCount/maxFrameCount;
  theta = TWO_PI*t;

  for ( var x= -width/2; x <= width/2; x += 25) {
    for (var y= -height/2; y <= height/2; y += 50) {
      var offSet = 80*x+y+y;   // play with offSet to change map below

      var x2 = map(sin(-theta+offSet), 0, 1, 0, 25); // map x position
      var y2 = map(cos(-theta+offSet), 0, 1, 25, 0); // map y position
      var x3 = map(sin(-theta+offSet*2), 0, 1, 0, 25); // map x position
      var y3 = map(cos(-theta+offSet*2), 0, 1, 25, 0); // map y position
      var sz2 = map(sin(-theta+offSet), 0, 1, 1, 5); // map size off the ellipse
      fill(250-(x/2), 250-(y/2), 150+(x/6)); // color with gradient created
      stroke(250-(x/2) - 10, 250-(y/2) - 10 , 150+(x/6) - 10)
     // ellipse(x+x2, y-y2, sz2, sz2);
     line(x+x2, y-y2, x+x2+x3, y-y2-y3)
    }
  }

}