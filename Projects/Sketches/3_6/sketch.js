...
function draw() {
  ...
  for (var p=0; p<num_particles; ++p) {
    ...
    noStroke();
    fill(particles[p].px * view_scale, 255, 255, 10);
    var cordx = particles[p].px * view_scale;
    var cordy = particles[p].py * view_scale;
    ellipse(cordx, cordy, 30, 30);
    particles[p].px = particles[p].x;
    particles[p].py = particles[p].y;
  }
  noStroke();
  fill(255);
  text("3.6.20", 30, height - 30);
}
