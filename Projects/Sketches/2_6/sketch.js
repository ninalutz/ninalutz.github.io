...
function setup() {
  ...
  for (let i = 0; i < 200; i++) {
    ...
    spin = new Spinner(createVector(width/2, height/2), random(0, 450), dir, 0, random(0.01, 0.1), random(0, 360));
    spins.push(spin);
  }
...
class Spinner{
 constructor(center, radius, direction, theta, vel, hue) {
    ...
    this.vel = vel;
    this.hue = hue;
    this.color = color(hue, 200, 200, 0.3);
