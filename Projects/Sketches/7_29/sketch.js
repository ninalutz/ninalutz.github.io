...
function draw() {
  if (_objectNum%2 == 0) {
    background(_aryColor[0]);
  } else {
    background(_aryColor[1]);
  }
  for (let i = 0; i < _objectNum; i++) {
...
class myObject {
  ...
  drawMe() {
    ...
    strokeWeight(2);
    for (let i = 0; i < _divideNum; i++) {
      line(this.centX + aryPoints[i][1] * cos(aryPoints[i][0]),
                  this.centY + aryPoints[i][1] * sin(aryPoints[i][0]), width/2, height/2);
    }
    for (let i = 0; i < 3; i++) {
      line(this.centX + aryPoints[i][1] * cos(aryPoints[i][0]),
                  this.centY + aryPoints[i][1] * sin(aryPoints[i][0]), width/2, height/2);
