...
function Circle(divideNum, rad, rad_offset, noiseSeedAng, noiseStepAng, noiseSeedRad, noiseStepRad) {
  ...
  let centX = width/2;
  let centY = height/2;
  let currentNoiseVal = noise(noiseSeedAng*2);
  ...
  for (let i = 0; i < divideNum+3; i++) {
    ...
    ellipse(centX + rad * (noise(noiseSeedRad + tgt_i * noiseStepRad)+rad_offset) * cos(2*PI*currentNoiseVal/sumNoiseVal),
      centX + rad * (noise(noiseSeedRad + tgt_i * noiseStepRad)+rad_offset) * sin(2*PI*currentNoiseVal/sumNoiseVal), 
      10*cos(2*PI*currentNoiseVal/sumNoiseVal), 
      10*cos(2*PI*currentNoiseVal/sumNoiseVal));
    rect(centX + rad * (noise(noiseSeedRad + tgt_i * noiseStepRad)+rad_offset) * 20*sin(2*PI*currentNoiseVal/sumNoiseVal), 
      centX + rad * (noise(noiseSeedRad + tgt_i * noiseStepRad)+rad_offset) * 20*cos(2*PI*currentNoiseVal/sumNoiseVal), 
      50*cos(2*PI*currentNoiseVal/sumNoiseVal), 
      50*cos(2*PI*currentNoiseVal/sumNoiseVal));
    }
  }
 // endShape();
