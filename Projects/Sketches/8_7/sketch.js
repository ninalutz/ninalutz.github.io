...
function Circle(divideNum, rad, rad_offset, noiseSeedAng, noiseStepAng, noiseSeedRad, noiseStepRad) {
  ...
  for (let i = 0; i < divideNum+3; i++) {
    ...
    ellipse(centX + rad * (noise(noiseSeedRad + tgt_i * noiseStepRad)+rad_offset) * cos(2*PI*currentNoiseVal/sumNoiseVal),
      centX + rad * (noise(noiseSeedRad + tgt_i * noiseStepRad)+rad_offset) * sin(2*PI*currentNoiseVal/sumNoiseVal), 10, 10);
    ellipse(centX + rad * (noise(noiseSeedRad + tgt_i * noiseStepRad)+rad_offset) * 20*sin(2*PI*currentNoiseVal/sumNoiseVal), 
      centX + rad * (noise(noiseSeedRad + tgt_i * noiseStepRad)+rad_offset) * 20*cos(2*PI*currentNoiseVal/sumNoiseVal), 20, 20);
    }
  }
 // endShape();
}
