...
function Circle(divideNum, rad, rad_offset, noiseSeedAng, noiseStepAng, noiseSeedRad, noiseStepRad) {
  ...
  for (let i = 0; i < divideNum; i++) {
    ...
    let rx2 = rad * (noise(noiseSeedRad + tgt_i * noiseStepRad)+rad_offset, sin(4*PI*currentNoiseVal/sumNoiseVal));
    let ry2 = rad * (noise(noiseSeedRad + tgt_i * noiseStepRad)+rad_offset, cos(4*PI*currentNoiseVal/sumNoiseVal));
