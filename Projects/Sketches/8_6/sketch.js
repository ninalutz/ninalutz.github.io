...
function Circle(divideNum, rad, rad_offset, noiseSeedAng, noiseStepAng, noiseSeedRad, noiseStepRad) {
  ...
  for (let i = 0; i < divideNum+3; i++) {
    ...
    line(centX + rad * (noise(noiseSeedRad + tgt_i * noiseStepRad)+rad_offset) * sin(2*PI*currentNoiseVal/sumNoiseVal),
      centY + rad * (noise(noiseSeedRad + tgt_i * noiseStepRad)+rad_offset) * cos(2*PI*currentNoiseVal/sumNoiseVal), 
      centX + rad * (noise(noiseSeedRad + tgt_i * noiseStepRad)+rad_offset) * 20*cos(2*PI*currentNoiseVal/sumNoiseVal), 
      centY + rad * (noise(noiseSeedRad + tgt_i * noiseStepRad)+rad_offset) * 20*sin(2*PI*currentNoiseVal/sumNoiseVal));
    }
    if (i%2 != 0){
        stroke((255*cos(2*PI*noise(noiseSeedRad))+140)%255);
    line(centX + rad * (noise(noiseSeedRad + tgt_i * noiseStepRad)+rad_offset) * cos(2*PI*currentNoiseVal/sumNoiseVal),
